...
/**
 * @brief   Dirección de la SDRAM donde comienza el espacio reservado para la
 * imágen en pantalla.
 * @ingroup GLCD
 */
...
/**
 * @brief   Parámetros de la LCD y ajustes indicados para el controlador de la
 * LCD del LPC4088.
 * @ingroup GLCD
 *
...
enum glcd_parametros {
  ...
  GLCD_VSYNC_FRONT_PORCH =
      8u, //!< Número de ciclos al finalizar la actualización de la pantalla.
  GLCD_VSYNC_BACK_PORCH =
      8u, //!< Número de ciclos al inicio de la actualización de la pantalla.
  ...
  GLCD_CICLOS_POR_LINEA =
      GLCD_PIXELES_POR_LINEA, //!< Num. ciclos de reloj por linea.
...
enum glcd_ctrl_bits {
  ...
  GLCD_CTRL_BYTEORDER_BIT =
      (1u << 9), //!< Formato de byte big-endian(1)/little-endian.
  GLCD_CTRL_BITORDER_BIT =
      (1u << 10),                //!< Orden de bits big-endian(1)/little-endian.
...
/**
 * @brief   Bits del registro POL (configuración del reloj) del módulo LCD del
 * LPC4088.
 * @ingroup GLCD
 */
enum glcd_pol_bits {
...
/**
 * @brief   Convertir el color RGB de 24 bits (8:8:8) al de resolución de 16
 * bits (5:6:5).
 * @ingroup GLCD
 *
...
/**
 * @brief   Constantes de los colores más comunes en español e inglés a partir
 * de sus valores RGB.
 * @ingroup GLCD
 */
enum glcd_color_t {
...
/**
 * @brief   Constantes para los diferentes tamaños de fuentes de caracteres
 * disponibles.
 * @ingroup GLCD
 */
enum fuentes_lcd {
...
// ===== GLCD - Tipos Publicos =====
/**
 * @brief   Estructura para almacenar la configuración de la LCD (color, fuente
 * y posición) para la salida de texto mediante glcd_printf.
 * @ingroup GLCD
 */
typedef struct {
  uint16_t color;
 ...
 * @details  Este dispositivo permite generar las señales de control de "reset"
 * y "display_enable", controlar la tensión de alimentación a la LCD (3.3V o 5V)
 * e intensidad de la retroiluminación de la pantalla LCD (genera una señal PWM
 * el propio PCA9532).
 * @{
 */
 ...
 * @details   Permite acceder a los registros secuencialmente sin reiniciar la
 * comunicación.
 */
...
enum glcd_pca9532_ls_modos {
  GLCD_LS_MODO_OFF =
      0x00, //!< Salida en alta impedancia (LED off). Valor por defecto.
...
/**
 * @brief   Constantes para generar las señales de control de la LCD con el
 * PCA9532.
 */
...
/**
 * @name      No Semihosting
 * @brief     Deshabilitar la funcionalidad de semihosting de ARM para el
 *compilador ARM v6 o v5.
 * @details   La funcionalidad de semihosting es un mecanismo de depuración que
 *permite que un dispositivo ARM target (embebido) se comunique con un host (PC)
 *para realizar operaciones de E/S, como printf, a través del depurador. Al
 *deshabilitar el semihosting, el compilador se asegura de que las funciones que
 *dependen de él no se incluyan, liberando memoria y eliminando la necesidad de
 *la conexión del host, lo cual es útil en entornos de producción o cuando se
 * desea una mayor independencia del host durante la depuración.
 *
 * Si se deshabilita el semihosting las funciones que lo utilizan se deben
 *reimplementar enviandolas a la UART, LCD u otras salidas. El Linker lanzará un
 *error indicando la función que utiliza semihosting que no se ha
 *reimplementado.
 *
 * Ejemplo:
 *
 ...
 * especificadores de formato que serán reemplazados por los valores de los
 * argumentos adicionales subsiguientes y formateados como se indique.
 * @param[in]   ...     Argumentos adicionales. Dependiendo de los
 * especificadores de formato incluidos en la cadena de formato, la función
 * espera una secuencia de argumentos adicionales cuyos valores se usan para
 * reemplazar dichos especificadores. Debe haber, al menos, tantos argumentos
 * adicionales como especificadores de formato. Los argumentos extra a los
 * necesarios son ignorados.
 *
 * @return  Si la función tiene éxito, retorna el número de caracteres que ha
 * impreso. En caso de error, se retorna un número negativo.
 */
int32_t glcd_printf(const char *format, ...);

/**
 * @brief   Función parecida a glcd_printf pero pudiendo indicar las coordedadas
 * donde aparece el texto, el color del texto y la fuente.
 * @ingroup GLCD
 *
 ...
 * opcionalmente especificadores de formato que serán reemplazados por los
 * valores de los argumentos subsiguientes y formateados como se indique.
 * @param[in]   ...           Argumentos adicionales. Dependiendo de los
 * especificadores de formato incluidos en la cadena de formato, la función
 * espera una secuencia de argumentos adicionales cuyos valores se usan para
 * reemplazar dichos especificadores. Debe haber, al menos, tantos argumentos
 ...
 * impreso. En caso de error, se retorna un número negativo.
 *
 * @note  No se actualiza la configuración actual de colores o posición de la
 * LCD.
 */
int32_t glcd_xprintf(uint16_t x, uint16_t y, uint16_t color,
                     uint16_t color_fondo, uint32_t fuente, const char *format,
                     ...);

/**
 * @brief   Imprime un carácter en pantalla. No se interpretan caracteres de
 * control.
 * @ingroup GLCD
 *
 * @param[in]   c             Carácter a imprimir. Sólo se imprimirá si es uno
 * de los caracteres definidos en la fuente indicada por el argumento fuente y
 * las coordenadas indicadas por los argumentos `x` e `y` están dentro de la
 * pantalla. En caso contrario no se imprimirá nada.
 * @param[in]   x             Coordenada X de pantalla en la que se imprimirá.
 * @param[in]   y             Coordenada Y de pantalla en la que se imprimirá.
 * @param[in]   color         Color con el que se imprimirá el carácter.
 * @param[in]   color_fondo   Color de fondo con el que se imprimirá el
 * carácter.
 * @param[in]   fuente        Fuente de caracteres con la que se imprimirá.
 */
void glcd_caracter(char c, uint16_t x, uint16_t y, uint16_t color,
                   uint16_t color_fondo, uint32_t fuente);

/**
 * @brief   Imprime una cadena de caracteres en la pantalla LCD. No se
 * interpretan los caracteres de control.
 * @ingroup GLCD
 *
 * @param[in]   x             Coordenada X de pantalla en la que se imprimirá.
 * @param[in]   y             Coordenada Y de pantalla en la que se imprimirá.
 ...
 * si las coordenadas indicadas por los argumentos `x` e `y` están dentro de la
 * pantalla. En caso contrario, no se imprimirá nada.
 */
...
/**
 * @brief   Fija el color de fondo que se usará para la salida de texto con
 * glcd_printf.
 * @ingroup GLCD
 *
...
/**
 * @brief   Fija las coordenadas en las que se mostrará el texto en la siguiente
 * llamada a `glcd_printf`. Sólo tiene efecto si las coordenadas están dentro de
 * la pantalla.
 * @ingroup GLCD
 *
...
/**
 * @brief   Desplaza hacia arriba el contenido de la pantalla un determinado
 * número de líneas. Las líneas inferiores se rellenan con el color del fondo de
 * texto actual.
 * @ingroup GLCD
 *
...
/**
 * @brief   Activa o desactiva el desplazamiento automático hacia arriba del
 * contenido de la pantalla cuando la salida de texto mediante glcd_printf
 * sobrepasa la linea inferior.
 * @ingroup GLCD
 *
...
/**
 * @brief   Selecciona la fuente de caracteres que se usará para la salida de
...
/**
 * @brief   Dibuja en la pantlla LCD un segmento de línea recta definido por las
 * coordenadas de sus extremos. Si las coordenadas de uno o ambos extremos del
 * segmento están fuera de la pantalla se dibuja la porción contenida en la
 * misma (si la hay).
 * @ingroup GLCD
 *
 ...
 */
void glcd_linea(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
                uint16_t color);

/**
 * @brief   Dibuja en la pantalla LCD un rectángulo definido por las coordenadas
 * de los vértices de una diagonal. Se dibuja la porción del rectángulo
 ...
 */
void glcd_rectangulo(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
                     uint16_t color);

/**
 * @brief   Dibuja en la pantalla LCD un rectángulo relleno definido por las
 * coordenadas de los vértices de una diagonal. Se dibuja la porción del
 * rectángulo contenida en los límites de la pantalla.
 * @ingroup GLCD
 *
 * @param[in]   x0      Coordenada X de un vértice.
 * @param[in]   y0      Coordenada Y de un vértice.
 ...
 * @param[in]   color   Color del rectángulo.
 */
void glcd_rectangulo_relleno(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
                             uint16_t color);

/**
 * @brief   Dibuja una circunferencia en la pantalla LCD. Se dibuja la parte de
 * la circunferencia contenida entre los límites de la pantalla.
 * @ingroup GLCD
 *
 ...
 */
void glcd_circunferencia(uint16_t xc, uint16_t yc, uint16_t radio,
                         uint16_t color);

/**
 * @brief   Dibuja un circulo relleno en la pantalla LCD. Se dibuja la parte del
 * cículo contenido entre los límites de la pantalla.
 * @ingroup GLCD
 *
 * @param[in]   xc      Coordenada X del centro.
 * @param[in]   yc      Coordenada Y del centro.
