 ...
 *
 * @details El LPC40xx tiene 5 UARTs. Las UARTs 0, 2 y 3 son exactamente iguales
 * entre sí. La UART 1 añade señales de control de modem y tiene registros
 * adicionales a los de las UARTs 0, 2 y 3. La UART 4 puede funcionar en un modo
 * síncrono y puede comunicar con smart cards, así que también tiene registros
 * adicionales.
 *
 * @note Las funciones de este módulo no usan ninguna de las características
 * especiales de las UARTs 1 y 4 por lo que se trataran como si fueran
 * "normales".
 *
 ...
 * @brief   Redefinición de la UART1 y la UART4 como UARTs de tipo
 * LPC_UART_TypeDef.
 *
 * @details Las funciones de este módulo no usan ninguna de las características
 ...
 * @ingroup UART
 * @note    No es obligatorio escoger uno de estos, aunque es recomendable por
 * compatibilidad.
 */
typedef enum {
...
/**
 * @brief   Tipo para el nivel de disparo de la FIFO de recepción que genera una
 * interrupción.
 * @ingroup UART
 */
typedef enum {
 ...
 * @ingroup UART
 *
 * @details En este registro se realizan los ajustes que definen el formato del
 * paquete de datos: número de bits, número de bits de stop, paridad y tipo.
 * Además tiene bits para deshabilitar la transmisión y habilitar el acceso a
 * los registros de divisor del reloj para establecer la frecuencia de la
 * transmision (baudrate).
 */
enum uart_lcr {
  ...
  UART_LCR_BREAK_CONTROL =
      (1u << 6), //!< Habilita el bloqueo de la transmisión.
  UART_LCR_ACCESO_DIVISOR_LATCH =
      (1u << 7) //!< Habilita el acceso a los registros DLL y DLM.
};
 ...
 * @ingroup UART
 *
 * @details En este registro se realizan los ajustes que controlan las FIFOs de
...
enum uart_fcr {
  ...
  UART_FCR_RXTRIGLVL =
      6 //!< Campo para el nivel de disparo de la FIFO de recepción.
};
 ...
 * @ingroup UART
 *
 * @details En este registro se habilitan o deshabilitan las interrupciones de
 * la UART.
 */
enum uart_ier {
  UART_IER_RBR =
      (1u << 0), //!< Interrupción cuando hay un nuevo dato disponible.
  UART_IER_THRE =
      (1u << 1), //!< Interrupción cuando la FIFO de transimisión está vacía.
...
enum uart_iir {
  UART_IIR_STATUS =
      (1u << 0), //!< Activo cuando hay al menos 1 interrupción pendiente.
  ...
  UART_IIR_ABTOINT =
      (1u << 9) //!< Activo si ocurrió un timeout durante el auto-baud.
};
...
void uart_inicializar(LPC_UART_TypeDef *uart_regs, uart_baudrate_t baudrate,
                      uart_bits_datos_t numero_bits_datos,
                      uart_paridad_t tipo_paridad,
